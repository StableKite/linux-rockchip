...
{
	struct rocket_iommu_domain *domain = kzalloc(sizeof(*domain), GFP_KERNEL);
	void *err;

	if (!domain)
		return ERR_PTR(-ENOMEM);

	domain->domain = iommu_domain_alloc(dev->bus);
...
static int rocket_remove(struct platform_device *pdev)
{
